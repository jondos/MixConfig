	...
	implements PKCSObjectIdentifiers
{
		...
		NoSuchProviderException, NoSuchAlgorithmException
	{
		...
		if (data.getCertificates() != null)
		{
			...
			{
				certs.addElement(JAPCertificate.getInstance(
					X509CertificateStructure.getInstance(ec.nextElement())));
			}
		}
		...
		{
			JAPCertificate cert = (JAPCertificate)certs.elementAt(i);
			if (serialNumber.equals(cert.getSerialNumber())
					&& issuer.equals(cert.getDistinguishedName()))
			{
				signCert = cert;
				break;
			}
		...
		NoSuchProviderException, NoSuchAlgorithmException
	{
		...
		//
		signCert = (JAPCertificate)certChain[0];
		for (int i = 0;i < certChain.length;i++)
		{
			certs.addElement(certChain[i]);
		}
	...
	{
		return (JAPCertificate[])certs.toArray(new JAPCertificate[0]);
	}
